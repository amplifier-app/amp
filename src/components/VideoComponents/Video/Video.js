...
import { AmplifierSocketProvider } from "../AmplifierSocketProvider";
import CallSidebar from "../CallSidedbar/CallSidebar";
...
export default function Video() {
	...
	const createCall = useCallback(async () => {
		...
		const tempRoomUrl = roomUrlFromPageUrl();
		if (tempRoomUrl) {
			// There's a room in the URL, connect them to that.
			return tempRoomUrl;
		} else {
			// Just connect them to a test room. only for dev
			try {
				const roomUrl = (await api.createRoom()).url;
				setRoomUrl(roomUrl);
			} catch (e) {
	...
	useEffect(() => {
		...
		//See if the URL is from auth0
		let params = new URL(document.location).searchParams;
		let authUrlState = params.get("code");
		//If URL is from auth0 then don't replace state
		if (authUrlState == null) {
		...
		<>
			...
			<div className="video">
				{showCall ? (
					...
					<Lobby setUserInfo={setUserInfo} join={() => createCall().then((url) => startJoiningCall(url))} />
				)}
			</div>
		</>
	);
