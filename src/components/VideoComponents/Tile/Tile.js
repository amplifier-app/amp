...
function getTrackUnavailableMessage(kind, trackState, props) {
...
export default function Tile(props) {
	...
	const audioTrack = useMemo(() => {
		return props.audioTrackState && props.audioTrackState.state === "playable" ? props.audioTrackState.track : null;
	...
	function getAudioComponent() {
		return !props.isLocalPerson && audioTrack && <audio autoPlay playsInline ref={audioEl} />;
	}
	...
	function getCornerMessageComponent() {
      ...
      // audioUnavailableMessage &&
      // !videoUnavailableMessage && <p className="corner">{audioUnavailableMessage}</p>
      <p className={"corner" + " small"}>
        {ellipsis(props.tileName, 24)}
      </p>
    );
	}
